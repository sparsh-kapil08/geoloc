    ...
    const aiResponse = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          make sure to cross verify the answer` },
        ]
      },
      config: {
      ...
      const aiResponse = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            { inlineData: { mimeType: 'image/jpeg', data: base64Image.split(',')[1] } },
            { text: `Locate this image. Be precise. prefer ${preference},search on web and Return a JSON object with lat, lng, city, country, confidence, and reasoning.
          make sure if the image dosen't have unique visuals,to the point names which defines the place. it looks like multiple places then give your response with the confidence in the range of 0.4 to 0.6
          the hints image got from the google lens=${serpResponse}
          make sure to cross verify the answer`}
          ]
        },
